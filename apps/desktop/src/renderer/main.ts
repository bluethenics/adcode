...
function rememberSession(): void {
  ...
  window.adcode.session.save({
    ...
    // localStorage is the fast path, but it is per-renderer-partition: a cleared or
    // fresh partition forgot Vibe and reopened in Code. The session file is the backup.
    workspaceMode: assistantDock?.mode() ?? (document.body.dataset["workspaceMode"] === "code" ? "code" : "vibe"),
  });
}
...
async function boot(): Promise<void> {
  ...
  /*
   * Reopening files must not change the working mode: restore flips nothing while
   * `sessionReady` is false, but anything after this point (recovery, a click) may
